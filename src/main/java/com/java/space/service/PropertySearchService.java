...
public class PropertySearchService {
	...
	//Search properties by combination of type, purpose and city. Each can expect 2 typos
	 public static List<Document> searchByTypePurposeAndCity(Directory index, Type type, Purpose purpose, String city) throws IOException, ParseException {
		...
		//Search properties by combination of type, purpose and city. Each can expect 2 typos
	 public static List<Document> searchByTimings(Directory index, Timings timing) throws IOException, ParseException {
		 IndexReader reader= DirectoryReader.open(index);
			IndexSearcher searcher=new IndexSearcher(reader);
	 ...
	 public static List<Document> searchBySpaceNameOrTitle(Directory index, String text) throws IOException, ParseException {
			IndexReader reader= DirectoryReader.open(index);
			IndexSearcher searcher=new IndexSearcher(reader);
		...
		public static void searchByAvailability(Directory index, Availability availability) throws IOException, ParseException {
			IndexReader reader= DirectoryReader.open(index);
			IndexSearcher searcher=new IndexSearcher(reader);
			Query q=new TermQuery(new Term("availabiltiy", availability.name()));//raw search query
			TopDocs docs=searcher.search(q, 10);
			Arrays.stream(docs.scoreDocs).map(x-> {
				try {
					return searcher.doc(x.doc);
				} catch (IOException e) {
					e.printStackTrace();
		...
		public static void searchByPopularity(Directory index, int popularity) throws IOException, ParseException {
			IndexReader reader= DirectoryReader.open(index);
			IndexSearcher searcher=new IndexSearcher(reader);
		...
		public static void searchByCategory(Directory index, Category category) throws IOException, ParseException {
			IndexReader reader= DirectoryReader.open(index);
			IndexSearcher searcher=new IndexSearcher(reader);
